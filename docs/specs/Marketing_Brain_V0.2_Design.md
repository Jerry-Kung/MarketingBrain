...
- 匹配表达式：`video_title LIKE '%#{object}%'`（前导 `#` 紧贴对象；标签之间以空格/换行分隔，尾部不再有 `#`，故不能要求尾 `#`。`#坦克300油耗`、`#坦克300改装` 等子话题同样命中，属预期）。

### 2.3 视频（来源）标识：`api_job.id`

- `api_job` 一行对应一次抓取作业，每个作业是同一视频下 **≤50 条评论的一个批次**；
  同一视频通常横跨数十个作业（实测：热门标题 32 个作业 / 1,510 条评论）。
- 因此「来源（视频）」的身份是 `video_title`；头部来源统计按 `video_title` 聚合
  （评论数、点赞和、作业数），并给出一个代表性 `job_id`（该标题下最小的 `api_job.id`）
  作为可回查的溯源 ID。按 `job_id` 排序头部来源无意义（全部封顶 50）。
- 评论的 `comment_id` 取展开后的 `comment_id`；报告里引用来源统一用 `job_id` + `video_title`。
